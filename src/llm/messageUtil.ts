...
export function createChatHistory(messages:LLMMessages, prompt:string) {
  ...
  for (const nShotExample of messages.nShotExamples) {
    chatHistory.push({role:'user', content:nShotExample.userMessage});
    chatHistory.push({role:'assistant', content:nShotExample.assistantMessage});
  }
